...
(function (document) {
    ...
    ReactDOM.render(React.createElement(ViewCtrl, { store: Store() }), document.getElementById('app'));
})(window.document);
...
module.exports = React.createClass({
    render: function () {
        return React.createElement(
            'div',
            { className: 'shape shape-' + this.props.type, onClick: this.props.onSelect },
            this.props.type
        );
    }
});
...
module.exports = () => {
    ...
    const getRandomElem = (arr, randomFn) => arr[(randomFn || getRandomIntExcluding)(arr.length)];
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement(
                    'h1',
                    null,
                    headerMap[this.state.winner]
                )
            ),
            React.createElement(
                'section',
                { className: 'opponent' },
                React.createElement(Shape, { type: this.state.shape2 || '?' })
            ),
            React.createElement(
                'section',
                { className: 'player' },
                shapes.map(shape => React.createElement(Shape, { type: shape, onSelect: () => onSelect(shape) }))
            ),
            React.createElement(
                'footer',
                null,
                React.createElement(
                    'button',
                    { className: 'btn-restart' + (gameOver ? ' btn-visible' : ''), onClick: onRestart },
                    'Play again?'
                )
            )
        );
